...
def trans_to_audacity(trans_string, outfilename):
    """Convert the space-separated transcription from frames2trans
    into Audacity's tab-separated label format."""
    lines = []
    for line in trans_string.strip().split('\n'):
        start, end, sym = line.split(' ', 2)
        lines.append(f"{start}\t{end}\t{sym}")
    with open(outfilename, 'w') as f:
        f.write('\n'.join(lines) + '\n')
