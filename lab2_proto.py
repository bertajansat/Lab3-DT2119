...
def forward(log_emlik, log_startprob, log_transmat):
    ...
    N, M = log_emlik.shape # N: Total frames. M: Total states
    ...
    # Recursion
    for t in range(1, N): # For each n
        for j in range(M):  # For each j
...
def backward(log_emlik, log_startprob, log_transmat):
    ...
    for t in range(N-2, -1, -1):
        for j in range(M):
            ...
            backward_prob[t, j] = logsumexp(log_transmat[j, :M] + log_emlik[t+1, :M] + backward_prob[t+1, :M]) # :M not needed on backwards and emlik, they are already NxM
